...
const std::string currentDateTime() {
 ...
 int yLowH = 0; int yHighH=83; int yLowS=175; int yHighS=255;int yLowV=136;int yHighV=255;
 int bLowH = 57; int bHighH=125; int bLowS=188; int bHighS=255;int bLowV=0;int bHighV=103;//=83;

 int bLastX=0;int bLastY=0;int yLastX=0;int yLastY=0;int rLastX=0;int rLastY=0;

 float degree_pid_output; float distance_pid_output;
 float center_x=0;float center_y=0;

 float degree_current_error=0;float degree_previous_error=0;
 float degree_p_term=0; int degree_p_coefficient=300;
 float degree_i_term=0; int degree_i_coefficient=10;float degree_i_collected=0;
 float degree_d_term=0; int degree_d_coefficient=90;

 float distance_current_error=0;float distance_previous_error=0;
 float distance_p_term=0; int distance_p_coefficient=0;
 float distance_i_term=0; int distance_i_coefficient=0;float distance_i_collected=0;
 float distance_d_term=0; int distance_d_coefficient=0;
 ...
 cvCreateTrackbar("ySatH", "Control", &yHighS, 255);*/
  ...
  Mat bImgThresholded;
   ...
   inRange(imgHSV, Scalar(bLowH, bLowS, bLowV), Scalar(bHighH, bHighS, bHighV), bImgThresholded); //Threshold the image for r
   inRange(imgHSV, Scalar(yLowH, yLowS, yLowV), Scalar(yHighH, yHighS, yHighV), yImgThresholded); //Threshold the image for r
 ...
 //END PID SECTION
  ...
  /*if ( abs(out_right) / out_right == -1 ){
    out_right = 256 + out_right;
  }
  if ( abs(out_left) / out_left == -1 ){
    out_left = 256 + out_left;
  }*/
  /*100% distance portion = farthest distance (b/t corners of screen)
  100% degree portion = +-180degrees
  -degrees, turn left; + turn right
  printf("%x",number); //this prints number in hexadecimal*/

/*  char* leftMessage[70] = {"0x0C 0x00 0x80 0x04 0x01 " + int_to_hex(out_left) + " 0x01 0x00 0x00 0x20 0x00 0x00 0x00 0x00"};
  serial.SendData(leftMessage, strlen(leftMessage));
  char* rightMessage[70] = {"0x0C 0x00 0x80 0x04 0x02 " + int_to_hex(out_right) + " 0x01 0x00 0x00 0x20 0x00 0x00 0x00 0x00"};
  serial.SendData(rightMessage, strlen(rightMessage));*/
	...
	while(SP->IsConnected())
	{
		...
		if (readResult!=-1){
            ...
            if(incomingData==0){
                ...
                char outgoingdata[5];
                sprintf(outgoingdata, "%d", rLastX);
                SP->WriteData(outgoingdata,dataLength);
                Sleep(5);
                sprintf(outgoingdata, "%d", rLastY);
                SP->WriteData(outgoingdata,dataLength);
                Sleep(5);
                sprintf(outgoingdata, "%d", bLastX);
                SP->WriteData(outgoingdata,dataLength);
                Sleep(5);
                sprintf(outgoingdata, "%d", bLastY);
                SP->WriteData(outgoingdata,dataLength);
                Sleep(5);
                sprintf(outgoingdata, "%d", yLastX);
                SP->WriteData(outgoingdata,dataLength);
                Sleep(5);
                sprintf(outgoingdata, "%d", yLastY);
                SP->WriteData(outgoingdata,dataLength);
                Sleep(5);
            }
            break;
